...
class CDVAE(BaseModule):
    ...
    def decode_stats(
        ...
        # print(
        #    "self.predict_num_atoms(z).argmax(dim=-1)",
        #    self.predict_num_atoms(z),
        #    self.predict_num_atoms(z).shape,
        # )
        # print(
        #    "self.predict_spg(z).argmax(dim=-1)",
        #    self.predict_spg(z),
        #    self.predict_spg(z).shape,
        # )
        spg = self.predict_spg(z).argmax(dim=-1) + 1
        return (
            num_atoms,
            lengths_and_angles,
            lengths,
    ...
    def langevin_dynamics(
        ...
        (
            ...
            composition_per_atom,
            spg,
        ) = self.decode_stats(z, gt_num_atoms)
        if gt_num_atoms is not None:
    ...
    def forward(self, batch, teacher_forcing, training):
        ...
        spg_loss = self.spg_loss(spg, batch)
    ...
    def sample_composition(self, composition_prob, num_atoms):
        ...
        for comp_prob, num_atom in zip(
            list(composition_prob), list(num_atoms)
        ):
    ...
    def predict_spg(self, z):
        return self.fc_spg(z)
    ...
    def num_atom_loss(self, pred_num_atoms, batch):
        # print("pred_num_Atoms", pred_num_atoms, pred_num_atoms.shape)
        # print("batch.num_atoms", batch.num_atoms, batch.num_atoms.shape)
        return F.cross_entropy(pred_num_atoms, batch.num_atoms)

    def spg_loss(self, pred_spgs, batch):
        # print("pred_spgs", pred_spgs, pred_spgs.shape)
        # print("batch.spg_numbs", batch.spg_numbs, batch.spg_numbs.shape)
        return F.mse_loss(
            pred_spgs.float(), batch.spg_numbs.view(-1, 1).float()
        )
    ...
    def compute_stats(self, batch, outputs, prefix):
        ...
        loss = (
            ...
            + self.hparams.cost_spg * spg_loss
